...
from models.resnet18.trasformazioni_1 import resnet18_data_transforms
...
class RepereKeypointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if image is None:
            raise FileNotFoundError(f"Immagine non trovata o illeggibile: {img_path}")
        ...
        ## sia ai keypoints grazie ad Albumentations
        ##
        transformed = self.transform(image=image, keypoints=keypoints)
        image = transformed["image"] # torch.Tensor (C, H, W) dopo ToTensorV2


        # Converto i keypoints in array prima della normalizzazione
        # (keypoints post-transform)
        keypoints = np.array(transformed["keypoints"], dtype="float32") # provare con il float64


        # Dimensioni reali post-transform
        altezza, larghezza = image.shape[1], image.shape[2]
